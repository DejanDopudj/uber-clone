...
export class MapComponent implements AfterViewInit {
  ...
  handlePassengerRideInProgress(): void {
    const passengerRide: RideSimple | null =
      this.passengerService.getCurrentRide();
    this.handleRideInProgress(passengerRide);
  }

  handleDriverRideInProgress(): void {
    const driverRide: DriverRide | undefined =
      this.driverService.getCurrentRides()?.currentRide;
    ...
    this.control = L.Routing.control({
      ...
      plan: L.Routing.plan([], {
        createMarker: function (i, wp) {
          return L.marker(wp.latLng, { draggable: true }).on(
            'contextmenu',
            function (e: any) {
    ...
    if (this.canUserAlterWaypoints()) {
      ...
      this.control.setWaypoints([
        ...this.control
          .getPlan()
          .getWaypoints()
          .filter((x: L.Routing.Waypoint) => x.latLng),
        e.latlng,
      ]);
      await this.reverseSearchLocation(e.latlng.lat, e.latlng.lng).then(
        (res) => {
    ...
    if (this.canUserAlterWaypoints()) {
      ...
      this.control.setWaypoints([
        ...this.control
          .getPlan()
          .getWaypoints()
          .filter((x: L.Routing.Waypoint) => x !== wp),
      ]);
  ...
  drawRideRoute(ride: RideSimple | DriverRide): void {
    ...
    const r: any = {
      name: 'Route',
      summary: {
        totalDistance: ride.distance * 1000,
        totalTime: ride.expectedTime,
      },
      coordinates: ride.route.coordinates?.map((latlng) => {
        return { ...latlng };
      }),
      waypoints: ride.route.waypoints?.map((latlng) => {
        return { ...latlng };
      }),
      inputWaypoints: ride.route.waypoints?.map((latlng) => {
        return { ...latlng };
      }),
    ...
    this.map.setView(
      [ride.route.waypoints[0].lat, ride.route.waypoints[0].lng],
      15
    );
  }
  ...
  drawVehiclePositions = () => {
    this.vehiclePositions.forEach((vehicle) => {
      ...
      if (!vehicle.rideActive) {
        ...
        const marker = L.marker(vehicle.currentCoordinates, {
          icon: this.unoccupiedTaxiIcon,
        }).addTo(this.map);
        this.vehicleMarkers[vehicle.id] = marker;
        if (this.vehicleRoutes[vehicle.id])
          delete this.vehicleRoutes[vehicle.id];
      } else {
        this.getRouteForCoordinates(vehicle);
        if (!this.vehicleMarkers[vehicle.id]) {
          const marker = L.marker(vehicle.currentCoordinates, {
            icon: this.occupiedTaxiIcon,
          }).addTo(this.map);
          this.vehicleMarkers[vehicle.id] = marker;
        }
        if (vehicle.rideActive) {
  ...
  getRouteForCoordinates = (vehicle: any) => {
    ...
    if (!this.vehicleRoutes[vehicle.id]) {
      const waypoint1 = new L.Routing.Waypoint(
        L.latLng(vehicle.currentCoordinates),
        '',
        {}
      );
      const waypoint2 = new L.Routing.Waypoint(
        L.latLng(vehicle.nextCoordinates),
        '',
        {}
      );
      L.Routing.control({
  ...
  simulateMovement = (vehicle: any): void => {
    ...
    const startingCoordinate =
      ~~(
        this.vehicleRoutes[vehicle.id].coordinates.length * finishedPartOfRide
      ) + 1;

    const coordinatesToDraw =
      this.vehicleRoutes[vehicle.id].coordinates.slice(startingCoordinate);
  ...
  canUserAlterWaypoints(): boolean {
    ...
    return (
      accountType === 'passenger' && !this.passengerService.getCurrentRide()
    );
  }

  async updateWaypoints(
    oldRouteWaypoints: any[],
    newRouteWaypoints: any[]
  ): Promise<void> {
    for (let i = 0; i < oldRouteWaypoints.length; i++) {
      if (
        !this.areSameCoordinates(
          oldRouteWaypoints[i].latLng,
          newRouteWaypoints[i].latLng
        )
      ) {
        await this.reverseSearchLocation(
          newRouteWaypoints[i].latLng.lat,
          newRouteWaypoints[i].latLng.lng
        ).then((res) => {
  ...
  fillWaypoints = async (ride: RideSimple | DriverRide) => {
    ...
    for (const latLng of ride.route.waypoints) {
      await this.reverseSearchLocation(latLng.lat, latLng.lng).then((res) => {
        this.waypoints.push(res);
      });
    }
  };
  ...
  setCursorStyle = () => {
    ...
    else document.getElementById('map')!.style.cursor = 'grab';
  };
}
